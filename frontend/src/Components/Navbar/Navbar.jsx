...
const Navbar = () => {
  ...
  return (
    ...
    {localStorage.getItem('auth-token')
     ?<Link to='/logout'><img onClick={()=>{localStorage.removeItem('auth-token');window.location.replace('/')}} title='Logout' className='img2' src={logout} alt="" /></Link>:<Link to='/login'><img title='Login' className='img2' src={login} alt="" /></Link>}
      
      <Link to='/cart'> <img className='cart-logo' src={cart_icon} alt="" /></Link>
